...
class PostPage extends Component {
  ...
  handleReplyUpvote = async (id) => {
    try {
      const replies_old = [...this.state.replies].reverse();
  ...
  render() {
    ...
    const { user } = this.props;
    const { content } = this.newState.Content;
    var question = content.find((element) => element.Type === 0);
    return (
      <div>
        <ToastContainer />
